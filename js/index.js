...
membersArray.forEach((member) => {
  ...
  h3.textContent = member.nickname || memberName;
    ...
    document.getElementById("members").appendChild(card);
})
...
async function loadResourcesForCourse(courseId, resourceType, section = 'all') {
  try {
    ...
    if (err.code === 'failed-precondition' || err.message.includes('index')) {
      console.warn('Index not available, trying without ordering');
      try {
        let query = db.collection('courses').doc(courseId).collection(resourceType);
...
function renderCoursesList() {
  ...
  coursesArray.forEach(course => {
    ...
    btn.addEventListener('click', async () => {
      ...
      await loadAndDisplayResources(course.id, currentActiveTab, currentActiveSection);
    });
    list.appendChild(btn);
  });
}

async function loadAndDisplayResources(courseId, resourceType, section = 'all') {
...
function renderResourcePanel(courseId, resourceType, resources, section = 'all') {
  ...
  const sectionText = section === 'all' ? 'All Sections' : `Section ${section}`;
  ...
  const header = `
    <div style="margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid rgba(0, 212, 255, 0.3);">
      ...
      <div style="color: rgba(0, 212, 255, 0.8); font-size: 13px; font-weight: 600;">
        📚 ${sectionText}
      </div>
    </div>
  `;
  ...
  if (!resources.length) {
    const resourceTypeName = resourceType === 'notes' ? 'Notes' : 
                            resourceType === 'slides' ? 'Lecture Slides' : 'Student Notes';
    const sectionName = section === 'all' ? 'any section' : `Section ${section}`;
    panel.innerHTML = header + `<div class="no-notes">No ${resourceTypeName.toLowerCase()} available for ${sectionName} yet.</div>`;
    return;
  }
  ...
  const resourcesHtml = resources.map(resource => {
    ...
    const sectionBadge = resource.section ? `<span style="background: rgba(0, 212, 255, 0.2); color: #00d4ff; padding: 2px 8px; border-radius: 10px; font-size: 11px; margin-left: 8px;">Section ${resource.section}</span>` : '';
    
    return `
      <div class="note-item">
        <div class="note-title">${safeTitle} ${sectionBadge}</div>
...
function clearAllResourcePanels() {
  const panels = ['course-notes', 'course-slides', 'course-student-notes'];
...
function switchResourceTab(tabName) {
  ...
  // Load resources for current course if one is selected
  if (currentSelectedCourse) {
    loadAndDisplayResources(currentSelectedCourse.id, tabName, currentActiveSection);
  }
}

function switchSectionFilter(section) {
  console.log('Section filter clicked:', section);
  ...
  // Load resources for current course if one is selected
  if (currentSelectedCourse) {
    console.log('Loading resources for course:', currentSelectedCourse.id, 'tab:', currentActiveTab, 'section:', section);
    loadAndDisplayResources(currentSelectedCourse.id, currentActiveTab, section);
  } else {
...
function openRoutinesModal() {
  ...
  // Load courses data
  loadCoursesFromFirebase().then(renderCoursesList);
  
  // Remove existing event listeners and set up tab switching
  document.querySelectorAll('.resource-tab').forEach(tab => {
  ...
  // Remove existing event listeners and set up section filter switching
  document.querySelectorAll('.section-filter').forEach(filter => {
    filter.removeEventListener('click', filter._sectionFilterHandler);
    filter._sectionFilterHandler = () => {
      switchSectionFilter(filter.dataset.section);
    };
    filter.addEventListener('click', filter._sectionFilterHandler);
  });
}
